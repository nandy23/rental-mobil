...
class C_Pesanan extends Controller
{
	...
	{
		if (!isset($id) || count($this->pesanan->cek($id)) == 0) redirect('pesanan');
		$pesanan = $this->pesanan->lihat_id($id);
		...
		$data = [
			'aktif' => 'pesanan',
			'judul' => 'Ubah Pesanan',
			'pemesan' => $this->pemesan->lihat_id($id_pemesan),
			'mobil' => $this->mobil->lihat_id($id_mobil),
			'perjalanan' => $this->perjalanan->lihat_id($id_perjalanan),
			'jenis_bayar' => $this->j_bayar->lihat_id($id_jenis_bayar),
			'pesanan' => $pesanan
		];
	...
	{
		if (!isset($id) || count($this->pesanan->cek($id)) == 0 || !isset($_POST['ubah'])) redirect('pesanan');
	...
	public function detail($id)
	{
		...
		$data = [
			...
			'pesanan' => $this->pesanan->detail($id)[0],
		];

		$this->view('pesanan/detail', $data);
		// var_dump($this->pesanan->detail($id));
	}
}
