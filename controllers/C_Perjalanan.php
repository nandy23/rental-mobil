...
class C_Perjalanan extends Controller
{
	...
	{
		if (!isset($id) || count($this->perjalanan->cek($id)) == 0) redirect('perjalanan');

		$data = [
			'aktif' => 'perjalanan',
			'judul' => 'Ubah Perjalanan',
			'perjalanan' => $this->perjalanan->lihat_id($id)
		];
	...
	{
		if (!isset($id) || count($this->perjalanan->cek($id)) == 0 || !isset($_POST['ubah'])) redirect('perjalanan');
