...
class C_Merk extends Controller
{
	...
	public function tambah()
	{
		...
		// var_dump($this->merk->tambah($merk));
		// exit;
	...
	{
		if (!isset($id) || count($this->merk->cek($id)) == 0) redirect('merk');

		$data = [
			'aktif' => 'merk',
			'judul' => 'Ubah Merk',
			'merk' => $this->merk->lihat_id($id)
		];
		$this->view('merk/ubah', $data);
		// var_dump(count($this->merk->cek($id)));
		// exit;
	}

	public function proses_ubah($id)
	{
		if (!isset($id) || count($this->merk->cek($id)) == 0 || !isset($_POST['ubah'])) redirect('merk');
	...
	{
		if (!isset($id) || count($this->mobil->cek($id)) == 0) redirect('merk');
