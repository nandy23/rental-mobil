...
class C_Mobil extends Controller
{
	...
	{
		if (!isset($id) || count($this->mobil->cek($id)) == 0) redirect('mobil');

		$data = [
			'aktif' => 'mobil',
			'judul' => 'Detail Mobil',
			'mobil' => $this->mobil->detail($id),
		];

		$this->view('mobil/detail', $data);
		// var_dump($this->mobil->detail($id));
		// exit;
	}
	...
	public function ubah($id)
	{
		...
		$data = [
			'aktif' => 'mobil',
			'judul' => 'Ubah Mobil',
			'mobil' => $this->mobil->lihat_id($id),
			'data_merk' => $this->merk->lihat()
		];
		$this->view('mobil/ubah', $data);

		// var_dump($this->mobil->lihat_id($id));
	}

	public function proses_ubah($id)
	{
		if (!isset($id) || count($this->mobil->cek($id)) == 0 || !isset($_POST['ubah'])) redirect('mobil');
		...
		$gambar_sebelumnya = $this->mobil->detail($id)['gambar'];
	...
	public function hapus($id = null)
	{
		...
		$gambar	= $this->mobil->detail($id)['gambar'];
