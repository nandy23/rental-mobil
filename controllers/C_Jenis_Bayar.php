...
class C_Jenis_Bayar extends Controller
{
	...
	{
		if (!isset($id) || count($this->j_bayar->cek($id)) == 0) redirect('jenis_bayar');

		$data = [
			'aktif' => 'jenis_bayar',
			'judul' => 'Ubah Jenis Bayar',
			'jenis_bayar' => $this->j_bayar->lihat_id($id),
		];
	...
	{
		if (!isset($id) || count($this->j_bayar->cek($id)) == 0 || !isset($_POST['ubah'])) redirect('jenis_bayar');
